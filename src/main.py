"""
Main application for the Financial Advisor Assistant.

This script initializes and runs the Financial Advisor application,
which uses a multi-agent architecture to provide specialized financial guidance.
"""
...
from aws_strands_poc.financial_advisor import FinancialAdvisor
...
def main():
    ...
    parser.add_argument(
        "--init_memory", 
        action="store_true",
        help="Initialize user memory with default preferences"
    )
    ...
    logger.info(f"MODEL environment variable: {os.environ.get('MODEL', 'Not set, using default')}")
    
    # On Windows, print a note about python_repl incompatibility
    if sys.platform == 'win32':
        logger.info("Running on Windows: python_repl tool will be disabled due to incompatibility")
    ...
    try:
        advisor = FinancialAdvisor(user_id=args.user_id, model=args.model)
    ...
    if args.init_memory:
        advisor.initialize_memory()
        print(f"Initialized memory for user: {args.user_id}")
    
    # Welcome message
    print("\n" + "="*80)
    print("💼 Financial Advisor Assistant 💼")
    print("="*80)
    print("\nThis application uses AWS Strands to provide specialized financial guidance.")
    print(f"Powered by OpenAI's {os.environ.get('MODEL', 'gpt-4o-mini')} model")
    print("\nAvailable specialists:")
    print("  - Market Analyst: For market trends, stock analysis, and economic outlook")
    print("  - Portfolio Manager: For investment strategy and portfolio optimization")
    print("  - Compliance Officer: For regulatory and legal guidance")
    print("  - Tax Specialist: For tax planning and calculations")
